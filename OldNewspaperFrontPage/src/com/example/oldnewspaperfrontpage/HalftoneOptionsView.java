 ...
 */
public class HalftoneOptionsView extends View {
	...
	public HalftoneOptionsView(Context context) {
		super(context);
		init(null, 0);
	}

	public HalftoneOptionsView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs, 0);
	}

	public HalftoneOptionsView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		final TypedArray a = getContext().obtainStyledAttributes(attrs,
				R.styleable.HalftoneOptionsView, defStyle, 0);

		mExampleString = a
				.getString(R.styleable.HalftoneOptionsView_exampleString);
		mExampleColor = a.getColor(
				R.styleable.HalftoneOptionsView_exampleColor, mExampleColor);
		// Use getDimensionPixelSize or getDimensionPixelOffset when dealing
		// with
		// values that should fall on pixel boundaries.
		mExampleDimension = a.getDimension(
				R.styleable.HalftoneOptionsView_exampleDimension,
				mExampleDimension);

		if (a.hasValue(R.styleable.HalftoneOptionsView_exampleDrawable)) {
			mExampleDrawable = a
					.getDrawable(R.styleable.HalftoneOptionsView_exampleDrawable);
